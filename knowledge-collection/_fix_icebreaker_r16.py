# -*- coding: utf-8 -*-
"""r16 修正：从累计墙移除本轮回填时已存在的 scavify 重复卡（墙内已有同名 URL 旧卡），
并对齐 tag 计数与 hero 文案；重建 .run_newcards.tmp.html（仅留 7 张净新增卡）。"""
import re, os
...
def card_html(c):
    return (
        ...
        f'<span class="cat">{c["cat"]}</span><span class="badge {c["rel"]}">{REL_LABEL[c["rel"]]}</span>'
        f'<span class="badge {c["src"]}">{SRC_LABEL[c["src"]]}</span></div>\n'
...
def remove_block_by_title(html, title):
    pos = html.find(f'<h3>{title}</h3>')
    assert pos != -1, f"title not found: {title}"
    s = html.rfind('<div class="hl">', 0, pos)
    assert s != -1
    i = html.index('>', s) + 1
    d = 1; j = i
    while j < len(html):
        ...
        if d == 0:
            break
    return html[:s] + html[j:]

html = open(WALL, encoding="utf-8").read()
before = html.count('<div class="hl">')
html = remove_block_by_title(html, "高管 Offsite 2 日议程·产出对齐与决策（决策块+RACI/DACI）")
after = html.count('<div class="hl">')
assert after == before - 1, f"block remove failed {before}->{after}"

html = html.replace('<span class="tag">50 卡</span>', '<span class="tag">49 卡</span>', 1)

old_hero = (' ｜ 十六轮补采 +8（2026-08-18 晚）：留任访谈 Stay Interview 11 问(skillhubs)/留任访谈轻量量表模板(stribehq)（②）；'
...
new_hero = (' ｜ 十六轮补采 +7（2026-08-18 晚；scavify 高管 Offsite 决策议程为累计墙已有卡，本轮不重复注入）：'
            '留任访谈 Stay Interview 11 问(skillhubs)/留任访谈轻量量表模板(stribehq)（②）；'
            '冲突后 90 分钟信任修复工作坊 SBI+Lencioni(unicornlabs)/重建团队凝聚力 7 步(archetype)（②）；'
            '高管 Offsite 五块+30 天强化(buildingteams)/领导力 Offsite 2.5 天外部引导师 ROI(bondeo)/领导团队凝聚力工作坊 2-3h+culturevitale 避坑（③）')
assert old_hero in html, "hero old text not found"
html = html.replace(old_hero, new_hero, 1)

open(WALL, "w", encoding="utf-8").write(html)

tmp = "".join(card_html(c) for c in CARDS)
...
n3 = html[:idx_sec2].count('<div class="hl">')
n2 = html[idx_sec2:idx_footer].count('<div class="hl">')
print(f"OK fix | wall hl {before}->{after} | sec3={n3} sec2={n2} | tmp cards={len(CARDS)}")
