# -*- coding: utf-8 -*-
"""
_icebreaker_r9.py — 破冰 第九轮补采 + 修复 R8 注入的 11 张损坏卡
1) 解析现有墙全部 .hl 卡；R8 损坏的 11 张（标题命中 FIX）用正确数据重建；
2) 追加本轮 7 张新卡（③2 / ②5，全二手，仅②③）；
3) 干净重排墙（head + hero + sec3 grid + sec2 grid + footer）；
4) 写 .run_newcards.tmp.html 供 gen_run_page 生成独立页。
"""
import re, os
...
# ---- R8 损坏 11 张的正确数据（标题 -> (val, how, url, disp)）----
FIX = {
 'Centering the C-Suite · 5 活动筑牢高管团队信任与对齐': (
 ...
 'Building a Healthy & Aligned Executive Team · 一日工作坊议程': (
 ...
 'Leadership Team Building That Actually Works · 7 个无游戏练习': (
 ...
 'Skip-Level Meeting Questions · 30 个建信任提问': (
 ...
 'The Skip-Level Meeting Playbook · 工程负责人实操手册': (
 ...
 'Skip-Level Questionnaire · 8 套模板（含心理安全感量表）': (
 ...
 'Align Cross-Functional Teams · 3 个实证做法': (
 ...
 'KPI-Driven Team Building · 把协调问题变可测实验': (
...
def field(b, cls, tag='span'):
    m = re.search(r'<'+tag+r' class="'+cls+r'">(.*?)</'+tag+r'>', b, re.S)
...
# 由 FIX 值(url)反查标题，实现「按真实 URL 命中」(规避标题中 · 等字符编码差异)
URL2TITLE = {v[2]: k for k, v in FIX.items()}
...
for b in blocks:
    ...
    if not href_v.startswith('http'):
        # 损坏卡：真实 URL 在链接文本里
        m = re.search(r'target="_blank">([^<]+)</a>', b)
        real = m.group(1).strip() if m else ''
        key = URL2TITLE.get(real)
        if key:
            val, how, url, disp = FIX[key]
            rel = 'exec' if 'badge r3' in b else 'supervisor'
            emoji = field(b, 'emoji'); cat = field(b, 'cat'); note = field(b, 'note', 'div')
            existing.append(card(emoji, key, cat, rel, val, how, url, disp, note))
            fixed += 1
            continue
    existing.append(b)
...
# 分类
def rel_of(b):
    if 'badge r3' in b: return 'r3'
    if 'badge r2' in b: return 'r2'
    return 'r2'
exist_r3 = [b for b in existing if rel_of(b) == 'r3']
exist_r2 = [b for b in existing if rel_of(b) == 'r2']
...
print('tmp new cards:', len(new_cards), '| new r3:', len(new_r3), '| new r2:', len(new_r2), '| fixed R8:', fixed)

# 重建墙
n3 = len(sec3); n2 = len(sec2)
hero = f'''  <div class="hero">
    <h1>🧊 破冰 · 知识采集卡片墙</h1>
    <p>采集于 2026-08-13 ｜ 九轮补采 +7（R9）｜ 六维评估（含关系适配度）｜ 一手/二手标注 ｜ 历史去重 ｜ 受众关系分层（仅②上下级 / ③高管间）｜ 本轮回修 R8 注入的 11 张损坏卡</p>
    <div class="relbar">
