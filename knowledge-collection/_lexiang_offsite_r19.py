# -*- coding: utf-8 -*-
"""乐享同步 . Offsite 十九轮（2026-08-19）。直连 streamable-http 客户端。
仅新建当轮独立页 offsite-20260819.html -> 子文件夹 463f5f53...(offsite)。
...
RUN_NAME = "offsite-20260819.html"
RUN_PATH = os.path.join(KC, "offsite", RUN_NAME)
...
def main():
    try:
        ...
        sm = mapf.setdefault("offsite", {"folder_id": FOLDER, "rounds": []})
        sm["folder_id"] = FOLDER
        sm["rounds"].append({"date": "20260819", "entry_id": rid, "name": RUN_NAME})
