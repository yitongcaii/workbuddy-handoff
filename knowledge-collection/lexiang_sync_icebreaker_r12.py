#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""乐享同步 · 破冰第十二轮（2026-08-15）。
直连 streamable-http 客户端（mcporter CLI 已失效）。
(b) 新建当轮独立页 icebreaker-2026-08-15-r12.html -> icebreaker 子文件夹 f51480b0
(a) 更新累计墙 icebreaker.html -> 既有新墙 entry 637b3b31（update 模式，file_id 3c5c8416）
回写 lexiang-entry-map.json：wall 改 637b3b31、rounds 追加 r12。
"""
...
RUN_NAME = "icebreaker-2026-08-15-r12.html"
RUN_DATE = "2026-08-15"
...
def main():
    ...
    # (b) 新建 R12 独立页
    run_path = os.path.join(KC, "icebreaker", "runs", RUN_NAME)
    ...
    ic["wall"] = {"entry_id": wall_entry, "file_id": wall_file, "name": WALL_NAME,
                  "note": "R12 累计墙（106卡）；update 模式持续刷新 637b3b31"}
    ic["rounds"].append({
        "date": RUN_DATE, "entry_id": rid, "name": RUN_NAME,
        "note": "轮次页 R12 (+9)"
    })
