# -*- coding: utf-8 -*-
"""乐享同步 · 破冰 第十三轮（2026-08-16）。
直连 streamable-http 客户端（mcporter CLI 已失效）。
(a) 更新累计墙 icebreaker.html -> 子文件夹 f51480b0 下 wall entry 637b3b31（in-place；若 403/404 则新建墙）
(b) 新建当轮独立页 icebreaker-2026-08-16-r13.html -> 子文件夹 f51480b0
...
SUBFOLDER = "f51480b0cfac4857bc28495b151c624f"
...
def main():
    ...
    try:
        print(f"\n=== 更新累计墙 icebreaker.html ({len(wall_data)}B) -> entry {WALL_ENTRY} ===")
        eid = update_wall(mc, WALL_ENTRY, WALL_FILE, "icebreaker.html", wall_data)
        print(f"    commit OK entry_id={eid} -> https://csig.lexiangla.com/pages/{eid}")
    except Exception as e:
        print(f"    ⚠️ 更新失败 ({str(e)[:200]})，回退为在子文件夹新建累计墙")
        eid = upload_new(mc, SUBFOLDER, "icebreaker.html", wall_data)
        used_entry = eid; used_file = None
        print(f"    新建墙 OK entry_id={eid} -> https://csig.lexiangla.com/pages/{eid}")

    # (b) 新建当轮独立页
    run_path = os.path.join(KC, "icebreaker", "runs", "icebreaker-2026-08-16-r13.html")
    run_data = open(run_path, "rb").read()
    run_name = "icebreaker-2026-08-16-r13.html"
    ...
    ib = mapf.setdefault("icebreaker", {"folder_id": SUBFOLDER, "wall": {}, "rounds": []})
    ib["folder_id"] = SUBFOLDER
    ib["wall"] = {"entry_id": used_entry, "file_id": used_file, "name": "icebreaker.html",
                  "note": "R13 累计墙（117卡）；update 模式持续刷新 637b3b31"}
    ib["rounds"].append({
        "round": "r13",
    ...
    print(f"\n=== 完成：累计墙已更新/新建 + 独立页已新建，已回写 lexiang-entry-map.json（rounds={len(ib['rounds'])}） ===")
