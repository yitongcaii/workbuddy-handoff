# -*- coding: utf-8 -*-
"""乐享同步 · Open Day 第十五轮（2026-08-15）。
直连 streamable-http 客户端（mcporter CLI 已失效）。
(a) 更新累计墙 openday.html -> 子文件夹 22eea86c 下 wall entry a6ba9c18（in-place；若 403/404 则新建墙）
(b) 新建当轮独立页 openday-2026-08-15-r15.html -> 子文件夹 22eea86c
...
def main():
    ...
    try:
        ...
        new_name = "openday.html"
        eid = upload_new(mc, SUBFOLDER, new_name, wall_data)
        used_entry = eid; used_file = None
        print(f"    新建墙 OK entry_id={eid} -> https://csig.lexiangla.com/pages/{eid}")

    # (b) 新建当轮独立页
    run_path = os.path.join(KC, "openday", "runs", "openday-2026-08-15-r15.html")
    run_data = open(run_path, "rb").read()
    run_name = "openday-2026-08-15-r15.html"
    ...
    sm["rounds"].append({
        "round": "r15",
        "date": "2026-08-15", "entry_id": rid, "name": run_name,
        "note": "轮次页 R15 (+15)"
    })
