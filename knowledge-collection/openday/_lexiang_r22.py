# -*- coding: utf-8 -*-
"""Open Day 第二十二轮（r22, 2026-08-22）· 乐享同步。
(a) 更新累计墙 openday.html -> 子文件夹 22eea86c 下 wall entry 443dbafc（in-place；403/404 兜底新建）
(b) 新建当轮独立页 openday-2026-08-22-r22.html -> 子文件夹 22eea86c
...
def main():
    ...
    try:
        ...
        eid = upload_new(mc, SUBFOLDER, "openday.html", wall_data)
        used_entry = eid; used_file = None
        print(f"    新建墙 OK entry_id={eid} -> https://csig.lexiangla.com/pages/{eid}")

    # (b) 新建当轮独立页
    run_path = os.path.join(KC, "openday", "runs", "openday-2026-08-22-r22.html")
    run_data = open(run_path, "rb").read()
    run_name = "openday-2026-08-22-r22.html"
    ...
    sm["wall"] = {"entry_id": used_entry, "file_id": used_file, "name": "openday.html",
                  "note": "R22 更新（entry 443dbafc）：累计墙 167 卡"}
    sm["rounds"].append({
        "round": "r22",
        "date": "2026-08-22", "entry_id": rid, "name": run_name,
        "note": "轮次页 R22 (+5：档案馆方志馆/美术馆艺术馆/疾控中心实验室/政务服务中心市民中心/融媒体中心广电总台开放日向·全②上下级，全一手)"
    })
